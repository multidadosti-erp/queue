...
class BaseImportImport(models.TransientModel):
	...
	def _split_file(self, model_name, translated_model_name,
		...
		if options.get(OPT_HAS_HEADER):
			...
			header_offset = 0
		# Mantem divisão padrão de 100 linhas por chunk.
		chunk_size = DEFAULT_CHUNK_SIZE
		batch_name = '%s (%s)' % (file_name, datetime.today())
		batch = self.env['queue.job.batch'].get_new_batch(batch_name)
		for chunk_index, (row_from, row_to) in enumerate(
				self._extract_chunks(model_obj, fields, data, chunk_size), start=1):
			chunk = str(priority - INIT_PRIORITY).zfill(padding)
			delay_seconds = chunk_index * CHUNK_DELAY_SECONDS
			...
			delayed_job = self.with_context(
				...
				description=description, priority=priority, eta=eta)._import_one_chunk(
